	...
	return function(RRule) {
		...
		ToText.isFullyConvertible = function(rrule) {
			...
			for (var key in rrule.origOptions) {
				...
				if (!contains(ToText.IMPLEMENTED[rrule.options.freq],
				key)) return false;
			}
		...
		ToText.prototype = {
			...
			toString: function() {
				...
				if (!this.isFullyConvertible()) this.add(
				gettext('(~ approximate)'));
			...
			MONTHLY: function() {
				...
				if (this.origOptions.bymonth) {
					if (this.options.interval !== 1) {
						...
						if (this.plural(this.options.interval)) this.add(
						gettext('in'));
					} else {
					...
					if (this.options.interval !== 1) this.add(
			...
			YEARLY: function() {
				...
				if (this.options.byyearday) {
					...
					add(this.list(this.options.byyearday, this.nth,
					gettext('and'))).
					add(gettext('day'));
				}
			...
			_bymonth: function() {
				this.add(this.list(this.options.bymonth, this.monthtext,
				this.gettext('and')));
			},
			...
			weekdaytext: function(wday) {
				var weekday = typeof wday === 'number'
				? wday
				: wday.getJsWeekday();
		...
		var parseText = function(text, language) {
			...
			function S() {
				...
				ttr.expect('every');
				if ((n = ttr.accept('number'))) options.interval = parseInt(
				n[0], 10);
			...
			function ON() {
				...
				} while (ttr.accept('comma') || ttr.accept('the') ||
				ttr.accept('on'));
			}
			...
			function decodeNTH() {
				switch (ttr.symbol) {
					...
					case 'nth':
						...
						if (v < -366 ||
						v > 366) throw new Error('Nth out of range: ' + v);
			...
			function F() {
				if (ttr.symbol === 'until') {
					...
					if (!date) throw new Error('Cannot parse until date:' +
					ttr.text);
