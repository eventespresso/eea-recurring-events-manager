...
;(function(root, factory) {
  ...
  var dateutil = {
    ...
    timeToUntilString: function(time) {
      ...
      for (var i = 0; i < comps.length; i++) {
        comp = comps[i];
        if (!/[TZ]/.test(comp) && comp < 10) comps[i] = '0' +
            String(comp);
      }
      return comps.join('');
    },
  ...
  var RRule = function(options, noCache) {
    ...
    if (invalid.length) throw new Error('Invalid options: ' +
        invalid.join(', '));
    ...
    if (opts.bysetpos !== null) {
      if (typeof opts.bysetpos ===
          'number') opts.bysetpos = [opts.bysetpos];
    ...
    if (!(plb(opts.byweekno) || plb(opts.byyearday) ||
      ...
      switch (opts.freq) {
        case RRule.YEARLY:
          if (!opts.bymonth) opts.bymonth = opts.dtstart.getMonth() +
              1;
      ...
      for (i = 0; i < opts.byhour.length; i++) {
        ...
        for (var j = 0; j < opts.byminute.length; j++) {
          ...
          for (var k = 0; k < opts.bysecond.length; k++) {
            ...
            // tzinfo=self._tzinfo))
            this.timeset.push(
                new dateutil.Time(hour, minute, second,
                    millisecondModulo));
          }
        }
      }
  ...
  RRule.optionsToString = function(options) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      if (value === null ||
          value instanceof Array && !value.length) continue;
      ...
      switch (key) {
        ...
        default:
          if (value instanceof Array) {
            for (j = 0; j <
            value.length; j++) strValues[j] = String(value[j])
            value = strValues;
          } else {
            value = String(value);
          }
    ...
    _iter: function(iterResult) {
      ...
      */
      ...
      if (freq < RRule.HOURLY) {
        ...
        if ((freq >= RRule.HOURLY && plb(byhour) &&
                !contains(byhour, hour)) ||
            (freq >= RRule.MINUTELY && plb(byminute) &&
                !contains(byminute, minute)) ||
            (freq >= RRule.SECONDLY && plb(bysecond) &&
                !contains(bysecond, minute))) {
          timeset = [];
        } else {
      ...
      while (true) {
        ...
        for (j = start; j < end; j++) {
          ...
          filtered = (plb(bymonth) &&
              !contains(bymonth, ii.mmask[i])) ||
              ...
              (plb(byyearday) &&
                  ((i < ii.yearlen &&
                      ...
                      (i >= ii.yearlen &&
                          ...
                          !contains(byyearday, -ii.nextyearlen + i -
                              ii.yearlen))));
        ...
        if (plb(bysetpos) && plb(timeset)) {
          ...
          for (j = 0; j < poslist.length; j++) {
            ...
            if (until && res > until) {
              ...
              ++total;
              if (!iterResult.accept(
                      res)) return iterResult.getValue();
              if (count) {
                --count;
                if (!count) {
                  this._len = total;
        ...
        if (fixday && day > 28) {
          ...
          if (day > daysinmonth) {
            while (day > daysinmonth) {
              ...
              daysinmonth = dateutil.monthRange(year, month -
                  1)[1];
            }
            ii.rebuild(year, month);
          }
        ...
        case 'BYSECOND':
          if (value.indexOf(',') !== -1) {
            ...
            for (j = 0; j < value.length; j++) {
              if (/^[+-]?\d+$/.test(value[j])) value[j] = Number(
                  value[j]);
            }
  ...
  Iterinfo.prototype.rebuild = function(year, month) {
    ...
    if (year !== this.lastyear) {
      ...
      if (!plb(rr.options.byweekno)) {
        ...
        no1wkst      = firstwkst = pymod(7 - this.yearweekday +
            rr.options.wkst, 7);
        if (no1wkst >= 4) {
          no1wkst = 0;
          // Number of days in the year, plus the days we got
          // from last year.
          wyearlen = this.yearlen +
              pymod(this.yearweekday - rr.options.wkst, 7);
        } else {
        ...
        if (no1wkst) {
          ...
          if (!contains(rr.options.byweekno, -1)) {
            var lyearweekday = dateutil.getWeekday(
                new Date(year - 1, 0, 1));
            var lno1wkst     = pymod(7 - lyearweekday +
                rr.options.wkst, 7);
            var lyearlen     = dateutil.isLeapYear(year - 1)
                ? 366
                : 365;
            if (lno1wkst >= 4) {
              lno1wkst  = 0;
              lnumweeks = Math.floor(52 +
                  pymod(lyearlen +
                      pymod(lyearweekday - rr.options.wkst, 7), 7) / 4);
            } else {
              lnumweeks = Math.floor(52 +
                  pymod(this.yearlen - no1wkst, 7) / 4);
            }
          } else {
            lnumweeks = -1;
          }
  ...
  Iterinfo.prototype.wdayset = function(year, month, day) {
    ...
    var i     = dateutil.toOrdinal(new Date(year, month - 1, day)) -
        this.yearordinal;
    var start = i;
    for (var j = 0; j < 7; j++) {
      set[i] = i;
  ...
  RRuleStr.prototype = {
    ...
    _parseRfc: function(s, options) {
      ...
      var exrulevals = [];
      var exdatevals = [];
      var name, value, parts, parms, parm, dtstart, rset, j, k, datestrs,
          datestr;

      if (!options.forceset && lines.length === 1 &&
          (s.indexOf(':') === -1 ||
              s.indexOf('RRULE:') === 0)) {
          ...
          if (name === 'RRULE') {
            ...
            for (j = 0; j < parms.length; j++) {
              ...
              if (parm !== 'VALUE=DATE-TIME') {
                throw new Error('unsupported RDATE parm: ' +
                    parm);
              }
            }
        ...
        if (options.forceset || rrulevals.length > 1 ||
            rdatevals.length ||
    ...
    parse: function(s, options) {
      ...
      defaultKeys.forEach(function(key) {
        if (!contains(keys,
                key)) options[key] = RRuleStr.DEFAULT_OPTIONS[key];
      });
  ...
  RRuleStr.prototype._handle_DTSTART = function(
      rrkwargs, name, value, options) {
